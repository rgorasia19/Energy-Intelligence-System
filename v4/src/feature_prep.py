...
def compute_rolling_fft_mag(series, window=48, freq_idx=1):
    # freq_idx 1 corresponds to ~ 1 cycle per 48 steps (daily if 30min data)
    # freq_idx 2 corresponds to ~ 2 cycles per 48 steps (12h)
    def extract_fft(x):
        # rfft returns complex numbers, take absolute magnitude
...
def create_features(df):
    """
    Create decoupled features for MoE.
    X_raw: for experts
    X_gate: for gating network (restricted to prevent shortcuts)
    """
    ...
    # Spectral (raw only)
    # 48 steps = 24 hours (assuming 30 min intervals)
    ...
    # Calendar features (both gate and raw can use this, but we'll prefix accordingly)
    ...
    # Regime Priors & Smoothed Base for Gating
    # The gating network gets smoothed macroscopic indicators instead of raw noisy data
...
def main():
    ...
    # Save column lists for train.py to use
