...
def run_ablation_test():
    ...
    with torch.no_grad():
        for x_raw, x_gate, y_label, y_vol, y_trend in test_loader:
            ...
            # 2. Ablated Forward Pass (Zero out Regime 2)
            p_regime_ablated = p_regime.clone()
            p_regime_ablated[:, :, 2] = 0.0 # Force Regime 2 probability to 0
            
            # Renormalize across remaining regimes (0 and 1)
            # Add a small epsilon to prevent division by zero in case a timestep was 100% Regime 2
    ...
    print("\n=============================================")
    print("  PHASE 2: ABLATION OF REGIME 2              ")
    print("=============================================")
    ...
    # Degradation specifically on timesteps where Regime 2 WAS dominant
    r2_mask = (hard_regimes == 2)
    if r2_mask.sum() > 0:
        ...
        print("Performance specifically on Regime 2 Timesteps:")
        ...
        print("  (If degradation is highly positive, Regime 2 is structurally vital.)")
    else:
        print("No Regime 2 dominant timesteps found to test specific degradation.")
        
    print("\n--- Generating Ablation Plot ---")
    
    if r2_mask.sum() > 0:
        # Plot the first 200 samples that belong to Regime 2
        idx_r2 = np.where(r2_mask)[0][:200]
        
        plt.figure(figsize=(12, 6))
        plt.plot(actual_targets_flat[idx_r2], label='Actual Demand', color='black', alpha=0.7)
        plt.plot(base_pred_flat[idx_r2], label='Baseline Prediction (K=3)', color='blue', alpha=0.7)
        plt.plot(ablated_pred_flat[idx_r2], label='Ablated Prediction (R2=0)', color='red', alpha=0.7)
        
        plt.title('Impact of Regime 2 Ablation on Target Timesteps')
        plt.xlabel('Sample Index (Filtered for Regime 2)')
        plt.ylabel('Demand')
        plt.legend()
        plt.savefig('ablation_regime2.png', dpi=600)
        print("Saved ablation plot to ablation_regime2.png")
