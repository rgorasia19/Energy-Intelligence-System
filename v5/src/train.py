...
def compute_regime_losses(p_regime, lambda1, lambda2, lambda3):
    ...
    # H(p) = - sum(p * log(p))
    # We want to MINIMIZE -H(p) which is equivalent to MAXIMIZING H(p)
...
def train():
    ...
    with mlflow.start_run() as run:
        ...
        for epoch in range(epochs):
            ...
            print(f"Epoch {epoch+1}/{epochs} - Tau: {tau:.2f} - Train Loss: {train_loss:.4f} - Val Pred Loss: {val_pred_loss:.4f}")
